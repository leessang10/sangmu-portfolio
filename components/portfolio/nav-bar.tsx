        ...
        {ROUTE_NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              className={`whitespace-nowrap rounded-full px-3 py-1.5 text-xs font-medium transition md:text-sm ${
                active ? "bg-cyan-300/15 text-cyan-300" : "text-slate-400 hover:text-slate-200"
              }`}
            >
              {item.label}
            </Link>
...
export function NavBar({ mode = "sections" }: NavBarProps) {
  return mode === "routes" ? <RouteNavBar /> : <SectionNavBar />;
}
